/*
 * LIN Master - Seat Button & LED Controller
 *
 * Controls heated and ventilated seat LEDs based on button presses
...
#define LIN_ID_DRIVER_LED       0xB1  // Driver LED control
#define LIN_ID_PASSENGER_BUTTON 0x80  // Passenger button press detection
#define LIN_ID_PASSENGER_LED    0x32  // Passenger LED control
...
// Driver LED control patterns (ID 0xB1)
...
// Passenger LED control patterns (ID 0x32)
...
void updateBoardLED() {
  if (flashActive) {
    if (millis() - flashStartTime < FLASH_DURATION) {
      ...
      // Flash complete, return to solid green
      flashActive = false;
...
void sendDriverLEDCommand() {
  ...
  // Determine which LED command to send based on current states
  if (driverHeatLevel != LED_OFF) {
    ...
    data[0] = DRIVER_LED_OFF;
  }
  data[1] = 0x00;

  // Send master request to control driver LEDs
...
void sendPassengerLEDCommand() {
  ...
  if (passengerHeatLevel != LED_OFF) {
    ...
    data[0] = PASSENGER_LED_OFF;
  }
  data[1] = 0x00;

  // Send master request to control passenger LEDs
...
void setup() {
  ...
  setLED(false, true, false);  // End with green
  ...
  // Print banner
  Serial.println("\n========================================");
  Serial.println("   LIN Master - Seat Button Controller");
  Serial.println("   2020 Ram 1500 Heated/Vent Seats");
  Serial.println("========================================");
